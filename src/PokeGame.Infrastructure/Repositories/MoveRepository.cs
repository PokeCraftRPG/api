...
internal class MoveRepository : Repository, IMoveRepository
{
  ...
  {
    return await base.LoadAsync<Move>(id.StreamId, isDeleted: false, cancellationToken);
  }
  public async Task<IReadOnlyCollection<Move>> LoadAsync(IEnumerable<MoveId> ids, CancellationToken cancellationToken)
  {
    return await base.LoadAsync<Move>(ids.Select(id => id.StreamId), isDeleted: false, cancellationToken);
  }
