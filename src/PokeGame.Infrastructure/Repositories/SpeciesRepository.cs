...
internal class SpeciesRepository : Repository, ISpeciesRepository
{
  ...
  {
    return await base.LoadAsync<PokemonSpecies>(id.StreamId, isDeleted: false, cancellationToken);
  }
  ...
  {
    return await base.LoadAsync<PokemonSpecies>(ids.Select(id => id.StreamId), isDeleted: false, cancellationToken);
  }
