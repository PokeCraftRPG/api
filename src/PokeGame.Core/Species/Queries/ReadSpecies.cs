...
internal class ReadSpeciesQueryHandler : IQueryHandler<ReadSpeciesQuery, SpeciesModel?>
{
  ...
  public ReadSpeciesQueryHandler(ISpeciesRepository speciesRepository)
  {
  ...
  public async Task<SpeciesModel?> HandleAsync(ReadSpeciesQuery query, CancellationToken cancellationToken)
  {
    ...
    {
      SpeciesModel? result = await _speciesRepository.ReadAsync(query.Id.Value, cancellationToken);
    ...
    {
      SpeciesModel? result = await _speciesRepository.ReadAsync(query.Number.Value, cancellationToken);
      if (result is not null)
      {
        species[result.Id] = result;
      }
    ...
    {
      SpeciesModel? result = await _speciesRepository.ReadAsync(query.Key, cancellationToken);
      if (result is not null)
      {
        species[result.Id] = result;
      }
