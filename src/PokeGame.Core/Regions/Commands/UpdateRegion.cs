...
internal record UpdateRegionCommand(Guid Id, UpdateRegionPayload Payload) : ICommand<RegionDto?>;

internal class UpdateRegionCommandHandler : ICommandHandler<UpdateRegionCommand, RegionDto?>
{
  ...
  public async Task<RegionDto?> HandleAsync(UpdateRegionCommand command, CancellationToken cancellationToken)
  {
    ...
    {
      region.SetKey(new Key(payload.Key), actorId);
    }
