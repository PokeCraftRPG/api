...
internal record UpdateAbilityCommand(Guid Id, UpdateAbilityPayload Payload) : ICommand<AbilityDto?>;

internal class UpdateAbilityCommandHandler : ICommandHandler<UpdateAbilityCommand, AbilityDto?>
{
  ...
  public async Task<AbilityDto?> HandleAsync(UpdateAbilityCommand command, CancellationToken cancellationToken)
  {
    ...
    {
      ability.SetKey(new Key(payload.Key), actorId);
    }
