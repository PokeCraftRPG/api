...
internal class PokemonService : IPokemonService
{
  ...
  public async Task<PokemonDto?> CatchAsync(Guid id, CatchPokemonPayload payload, CancellationToken cancellationToken)
  {
  ...
  public async Task<PokemonDto?> ReceiveAsync(Guid id, ReceivePokemonPayload payload, CancellationToken cancellationToken)
  {
    ReceivePokemonCommand command = new(id, payload);
    return await _commandBus.ExecuteAsync(command, cancellationToken);
  }

  public async Task<PokemonDto?> ReleaseAsync(Guid id, CancellationToken cancellationToken)
  {
  ...
  public async Task<PokemonDto?> WithdrawAsync(Guid id, CancellationToken cancellationToken)
  {
    WithdrawPokemonCommand command = new(id);
    return await _commandBus.ExecuteAsync(command, cancellationToken);
  }
}
