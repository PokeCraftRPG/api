...
public interface IWorldService
{
  ...
  Task<WorldDto?> ReadAsync(Guid? id = null, string? key = null, CancellationToken cancellationToken = default);
  Task<SearchResults<WorldDto>> SearchAsync(SearchWorldsPayload payload, CancellationToken cancellationToken = default);
  Task<WorldDto?> UpdateAsync(Guid id, UpdateWorldPayload payload, CancellationToken cancellationToken = default);
}
...
internal class WorldService : IWorldService
{
  public static void Register(IServiceCollection services)
  {
    ...
    services.AddTransient<ICommandHandler<UpdateWorldCommand, WorldDto?>, UpdateWorldCommandHandler>();
    services.AddTransient<IQueryHandler<ReadWorldQuery, WorldDto?>, ReadWorldQueryHandler>();
    services.AddTransient<IQueryHandler<SearchWorldsQuery, SearchResults<WorldDto>>, SearchWorldsQueryHandler>();
  }
  ...
  public async Task<WorldDto?> ReadAsync(Guid? id, string? key, CancellationToken cancellationToken)
  {
  ...
  public async Task<WorldDto?> UpdateAsync(Guid id, UpdateWorldPayload payload, CancellationToken cancellationToken)
  {
