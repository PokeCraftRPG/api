...
public class AddInventoryItemCommandHandlerTests
{
  ...
  private readonly AddInventoryItemCommandHandler _handler;

  public AddInventoryItemCommandHandlerTests()
  {
    _context = new(_faker);
    _handler = new(_context, _inventoryRepository.Object, _itemQuerier.Object, _itemRepository.Object, _trainerRepository.Object);
  }

  [Theory(DisplayName = "It should add item(s) to the inventory.")]
  [InlineData(null)]
  [InlineData(0)]
  [InlineData(2)]
  public async Task Given_TrainerAndItem_When_HandleAsync_Then_InventoryItemAdded(int? existingQuantity)
  {
    ...
    AddInventoryItemCommand command = new(trainer.EntityId, item.EntityId, payload);
  ...
  public async Task Given_ItemNotFound_When_HandleAsync_Then_ItemNotFoundException()
  {
    ...
    AddInventoryItemCommand command = new(trainer.EntityId, Guid.NewGuid(), payload);
  ...
  {
    InventoryQuantityPayload payload = new(quantity: 1);
    AddInventoryItemCommand command = new(Guid.NewGuid(), Guid.NewGuid(), payload);
