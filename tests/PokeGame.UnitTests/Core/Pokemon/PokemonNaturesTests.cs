...
{
  private IReadOnlyDictionary<string, NatureRecord> _expected = new Dictionary<string, NatureRecord>();
  ...
  public async Task InitializeAsync()
  {
    ...
    csv.Context.RegisterClassMap<NatureRecord.Map>();

    IAsyncEnumerable<NatureRecord> records = csv.GetRecordsAsync<NatureRecord>();

    Dictionary<string, NatureRecord> expected = [];
    await foreach (NatureRecord record in records)
    {
  ...
  {
    foreach (KeyValuePair<string, NatureRecord> item in _expected)
    {
  ...
  public void Given_Keys_When_get_Then_CorrectNatures()
  {
    ...
    {
      NatureRecord info = _expected[nature.Name.Trim().ToLowerInvariant()];
