...
public class CatchPokemonCommandHandlerTests
{
  ...
  private readonly CatchPokemonCommandHandler _handler;
  ...
  public CatchPokemonCommandHandlerTests()
  {
    _context = new(_faker);
    _handler = new(
      _context,
  ...
  {
    CatchPokemonPayload payload = new(_trainer.Key.Value, _pokeBall.Key.Value, "Mt. Coronet");
    CatchPokemonCommand command = new(_specimen.EntityId, payload);
  ...
  {
    CatchPokemonPayload payload = new(_trainer.Key.Value, _pokeBall.Key.Value, "Mt. Coronet");
    CatchPokemonCommand command = new(Guid.NewGuid(), payload);
