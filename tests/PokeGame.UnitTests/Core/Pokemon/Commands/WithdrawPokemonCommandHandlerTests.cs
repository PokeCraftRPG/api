...
public class WithdrawPokemonCommandHandlerTests
{
  ...
  private readonly WithdrawPokemonCommandHandler _handler;
  ...
  public WithdrawPokemonCommandHandlerTests()
  {
    _context = new(_faker);
  ...
  [Fact(DisplayName = "It should withdraw the Pokémon.")]
  public async Task Given_Pokemon_When_HandleAsync_Then_Withdrawd()
  {
    ...
    WithdrawPokemonCommand command = new(_specimen.EntityId);
    PokemonModel? result = await _handler.HandleAsync(command, _cancellationToken);
    Assert.NotNull(result);
    Assert.Same(model, result);

    _permissionService.Verify(x => x.CheckAsync(Actions.Withdraw, _specimen, _cancellationToken), Times.Once());
    ...
    Assert.Equal([specimen.Id, _specimen.Id], roster.GetParty());
  }

  [Fact(DisplayName = "It should return null when the Pokémon was not found.")]
  public async Task Given_NotFound_When_HandleAsync_Then_NullReturned()
  {
    WithdrawPokemonCommand command = new(Guid.NewGuid());
