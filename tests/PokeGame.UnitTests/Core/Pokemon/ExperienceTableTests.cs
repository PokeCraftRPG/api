...
{
  private LevelExperienceRecord[] _expected = [];
  ...
  public async Task InitializeAsync()
  {
    ...
    csv.Context.RegisterClassMap<LevelExperienceRecord.Map>();

    IAsyncEnumerable<LevelExperienceRecord> records = csv.GetRecordsAsync<LevelExperienceRecord>();

    List<LevelExperienceRecord> items = new(capacity: 100);
    await foreach (LevelExperienceRecord record in records)
    {
      items.Add(record);
    }
