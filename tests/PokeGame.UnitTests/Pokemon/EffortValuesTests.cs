...
public class EffortValuesTests : UnitTests
{
  ...
  public void Given_EmptyTraining_When_Create_Then_ZeroEffortValues()
  {
    ...
    Assert.Equal((byte)0, effortValues.Vitality);
    Assert.Equal((byte)0, effortValues.Stamina);
  ...
  [Fact(DisplayName = "It should return zero effort values from the parameterless constructor.")]
  public void Given_Parameterless_When_Create_Then_ZeroEffortValues()
  {
  ...
  public void Given_SkillTraining_When_Create_Then_Mapped()
  {
    ...
    {
      [PokemonSkill.Survival] = new(level: 0, rank: 2),    // effective 1 → Vitality 18
      [PokemonSkill.Athletics] = new(level: 1, rank: 2),   // effective 3 → Stamina 54
      ...
      [PokemonSkill.Perception] = new(level: 4, rank: 10), // ignored for EVs
      [PokemonSkill.Stealth] = new(level: 4, rank: 10) // ignored for EVs
    };

    EffortValues effortValues = new(training);

    Assert.Equal((byte)18, effortValues.Vitality);
    Assert.Equal((byte)54, effortValues.Stamina);
